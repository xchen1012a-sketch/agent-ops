...
- 阶段：`LEGAL-100`
- 名称：法律咨询 Agent 业务实现
- 状态：`LEGAL-150 领域 API 与课件功能` 第九切片已落地：咨询记录列表/搜索 API 契约
...
- 当前执行入口：
  ...
  - Implemented LEGAL-150 consultation record list/search API: `GET /v1/consultation-records`; returns user-owned paginated history projections.
- 暂不具备 / 后置依赖：
  - 课件法律知识库样本未提供；第一切片仅实现 mock/adapter 边界。
  - 知识材料导入、Qdrant 索引、BGE embedding/reranker、真实 RAG 检索和 DeepSeek 真实问答仍未开始。
- 下一步：
  - Continue LEGAL-150 later thin slices: admin high-risk review list/resolve API, then quality acceptance; keep DeepSeek/RAG and frontend as separate slices.
