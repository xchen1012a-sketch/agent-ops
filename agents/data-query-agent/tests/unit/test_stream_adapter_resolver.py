...
async def test_enabled_without_key_falls_back_to_fake() -> None:
    master = Fernet.generate_key().decode()
    reader = _FakeReader(_config(enabled=True, api_key_encrypted=None))
...
async def test_missing_master_key_falls_back_to_fake() -> None:
    master = Fernet.generate_key().decode()
    encrypted = ApiConfigCrypto(master).encrypt("sk-live-key-1234")
    reader = _FakeReader(_config(enabled=True, api_key_encrypted=encrypted))

    # Settings has an empty master key → crypto construction fails → fallback.
    adapter = await resolve_stream_adapter(subject="user-1", reader=reader, settings=_settings(""))
...
async def test_undecryptable_token_falls_back_to_fake() -> None:
    ...
    # Resolver uses a different master key than the one that encrypted the token.
    adapter = await resolve_stream_adapter(
