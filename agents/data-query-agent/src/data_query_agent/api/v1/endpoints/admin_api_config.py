...
from data_query_agent.api.v1.schemas.agent_api_config import (
...
async def list_api_configs(
    ...
    items = await repo.list_for_user(subject)
    views = {_item.api_type: _to_view(_item) for _item in items}
    for api_type in _API_TYPE_DEFAULTS:
        views.setdefault(api_type, _empty_view(subject, api_type))
...
async def get_api_config(
    ...
    entity = await repo.get_by_type(subject, api_type)
    if entity is None:
...
async def update_api_config(
    ...
    existing = await repo.get_by_type(subject, api_type)
