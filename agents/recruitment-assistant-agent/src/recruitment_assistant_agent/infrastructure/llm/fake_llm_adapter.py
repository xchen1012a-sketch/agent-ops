...
Produces mock thinking + analysis chunks so the recruitment run stream can render
the Claude.ai-style reasoning panel. Real DeepSeek streaming lands in 阶段5.
"""
...
from recruitment_assistant_agent.domain.ports.llm_adapter import (
    LlmAdapter,
...
class FakeLlmAdapter(LlmAdapter):
    ...
    async def stream(self, request: LlmCompletionRequest) -> AsyncIterator[LlmStreamChunk]:
        """Yield deterministic thinking then analysis chunks for local streaming demos."""
        self.requests.append(request)
...
def _default_thinking(request: LlmCompletionRequest) -> str:
    return (
        f"先解析岗位与简历要点：{request.rendered_prompt.strip()}。"
        "逐条比对能力证据，评估匹配度与差距，再组织分析结论与面试问题。"
    )
...
def _default_answer(request: LlmCompletionRequest) -> str:
    return (
        "### 人岗匹配分析\n\n"
        "这是一段用于本地演示的确定性分析占位，用于验证思考展示与正文流式渲染链路。\n\n"
        "- 匹配证据逐条列出\n"
        "- 差距点与建议面试问题随后给出\n"
    )
